...
interface ViewModeSwitcherProps {
  ...
  onMapClick?: () => void; // Optional: Navigate to map page instead of changing mode
  onChatbotOpen?: () => void; // Optional: Open chatbot
  className?: string;
}
...
const ViewModeSwitcher = ({ currentMode, onModeChange, onMapClick, onChatbotOpen, className }: ViewModeSwitcherProps) => {
  ...
  return (
    <div className={cn("flex items-center gap-1 bg-muted/50 p-1 rounded-lg", className)}>
        ...
        return (
          <button
            ...
            onClick={(e) => {
              e.preventDefault();
              e.stopPropagation();
              // Special handling for map mode: navigate to map page if onMapClick provided
      ...
      {/* AI Chatbot Button - Mobile only, replaces Match on mobile */}
      <button
        onClick={(e) => {
          e.preventDefault();
          e.stopPropagation();
