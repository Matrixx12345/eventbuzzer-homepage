...
interface DiscoverHeroFilterBarProps {
  ...
  // Callbacks
  onCategoryChange: (category: typeof categories[0]) => void;
...
const DiscoverHeroFilterBar = ({
  ...
  onDateChange,
  onSearch,
}: DiscoverHeroFilterBarProps) => {
  // Filter states
  const [selectedCategory, setSelectedCategory] = useState(() => 
    categories.find(c => c.slug === initialCategory) || categories[0]
  );
  const [selectedMood, setSelectedMood] = useState(() => 
    moods.find(m => m.slug === initialMood) || moods[0]
  );
  ...
  return (
    <div className="w-full flex justify-center px-4 md:px-6">
      ...
      <div className="backdrop-blur-xl bg-white/25 border border-white/40 rounded-2xl lg:rounded-full p-3 md:p-4 lg:px-6 lg:py-3 shadow-2xl w-fit max-w-full">
...
export default DiscoverHeroFilterBar;

export { categories, moods };
