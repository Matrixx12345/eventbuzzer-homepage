...
import { X, ChevronLeft, Send, Sparkles, Loader2, MapPin, Navigation, MessageCircle } from "lucide-react";
...
type ChatbotContext = "homepage" | "event-finder" | "trip-planner";
...
const CONTEXT_PROMPTS: Record<ChatbotContext, string> = {
  homepage: "Du hilfst bei allgemeinen Fragen zur Schweiz und Events. Sei freundlich und inspirierend.",
  "event-finder": "Du hilfst beim Finden einzelner Events. Der User ist auf der Event-Finder Seite. Gib konkrete Event-Empfehlungen basierend auf dem aktuellen Kartenbereich.",
  "trip-planner": "Du hilfst beim Planen mehrtägiger Touren in der Schweiz. Optimiere Routen, schlage Alternativen vor und beachte Reisezeiten.",
};

const CONTEXT_WELCOME: Record<ChatbotContext, string> = {
  homepage: "Hi! 👋 Verrate mir deinen Wunsch oder lass uns das Richtige über mein Quiz finden! ✨",
  "event-finder": "Hi! 👋 Ich helfe dir, das perfekte Event zu finden. Frag mich einfach!",
  "trip-planner": "Hi! 👋 Ich helfe dir, deinen Trip zu optimieren. Was möchtest du anpassen?",
};
...
const Chatbot = ({ 
  ...
  const [step, setStep] = useState<WizardStep>(context === "homepage" ? "mission" : "complete");
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { role: "bot", content: CONTEXT_WELCOME[context] },
  ]);
  ...
  const sendMessageToAI = async (userContent: string) => {
    ...
    const systemContext = CONTEXT_PROMPTS[context];
    const fullPrompt = `[Context: ${systemContext}]\n\nUser: ${userContent}`;
  ...
  const resetWizard = () => {
    setStep(context === "homepage" ? "mission" : "complete");
    setSelectedMission(null);
    setSelectedTime(null);
    setSelectedDate(undefined);
    setLocationInput("");
    setSelectedRadius(null);
    setMessages([{ role: "bot", content: CONTEXT_WELCOME[context] }]);
  };

  // Bubble variant for event-finder and trip-planner
  if (variant === "bubble") {
    return (
      <>
        ...
        {!isOpen && (
          <button
            onClick={handleOpen}
            className="fixed bottom-6 right-6 z-50 w-14 h-14 rounded-full bg-[#FDFBF7] shadow-lg flex items-center justify-center text-2xl hover:scale-110 transition-transform border-2 border-gray-200"
        ...
        {/* Chat Drawer */}
        {isOpen && (
          <div className="fixed bottom-6 right-6 z-50 w-[380px] max-w-[calc(100vw-2rem)] bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden animate-in slide-in-from-bottom-4 duration-300">
            {/* Header */}
            <div className="flex items-center justify-between p-4 border-b border-gray-100 bg-[#FDFBF7]">
              <h3 className="font-semibold text-gray-900">
                {context === "event-finder" ? "Event-Assistent" : "Trip-Assistent"}
              </h3>
            ...
            {/* Messages */}
            <div className="h-[300px] overflow-y-auto p-4 space-y-3 bg-white">
                  ...
                  <div
                    className={`max-w-[85%] px-4 py-2.5 rounded-2xl text-sm ${
                      message.role === "user"
                        ...
                        : "bg-[#FDFBF7] text-gray-800 rounded-bl-md border border-gray-100"
                    }`}
                  >
                    {message.content}
                  </div>
              ...
              {isLoading && messages[messages.length - 1]?.role === "user" && (
                <div className="flex justify-start">
                  <div className="bg-[#FDFBF7] text-gray-800 rounded-2xl rounded-bl-md border border-gray-100 px-4 py-2.5">
            ...
            <div className="p-4 border-t border-gray-100 bg-white">
              <div className="flex gap-2">
                <Textarea
                  ...
                  placeholder={context === "event-finder" 
                    ? "z.B. 'Zeig mir Museen in der Nähe'" 
                    : "z.B. 'Mach weniger Fahrt'"
                  }
                  disabled={isLoading}
  ...
  // Sidebar variant (original homepage style)
  return (
    <>
      {/* Chevron Handle */}
      {!isOpen && (
            ...
            <div className="p-4 pt-2">
              <div className="flex gap-2 items-end">
                <Textarea
                  ...
                  placeholder="Ich möchte diesen Samstag in ein Museum in Basel gehen."
                  disabled={isLoading}
